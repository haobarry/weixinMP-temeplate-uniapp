...
const Graphics = function(ctx, width, height) {
  ...
  this.area = {
    ...
    width: width - this.padding_width * 2,
    height: height - this.border_size * 2,
    // top: this.border_size - 4,
    // left: this.padding_width + this.quiet
    top: this.border_size,
    left: this.padding_width,
  };
...
//绘制条形码
const code128 = function(ctx, text, width, height, rotateObj, callBackFn) {

  // 旋转选项
  if (rotateObj.isRotate) {
    ctx.translate(height, 0)
    ctx.rotate(rotateObj.degree * Math.PI / 180)
...
function barc({id, component}, code, width, height, {isRotate = false, degree = 90} = {}, callBackFn) {
  // console.log('🎈 arguments>>>', arguments)
  code128(uni.createCanvasContext(id, component), code, convert_length(width), convert_length(height), {
    isRotate,
    degree,
  }, callBackFn)
}

export {barc as wxBarCode}